...
exports.getRetrieve = function (req, res) {
...
exports.activate = function (req, res) {
    ...
    if( mail && passMd5 === newPassMd5 ){
        pageUser.activeUser(mail, function (err, result) {
            if( err ) res.status( 500 ).send(err);
            if( !err ) res.send('您已激活账户：'+ mail);
        });
    }else{
...
exports.login = function (req, res) {
    ...
    pageUser.login(mail, password, function (err, result) {
        if( err ){
            ...
            result && result.avatar = 'http://en.gravatar.com/avatar/'+ tool.getMd5( result.mail ) +'?size=100';
