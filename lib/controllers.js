...
exports.setSession = function (req, res, next) {
    ...
    if (!req.cookies[ 'demos_session' ]) {
        res.cookie( 'demos_session', userID, { expires: new Date(Date.now() + 86400000*30), httpOnly: true  });
    }
    if (!req.cookies[ 'userID' ]) {
        res.cookie( 'userID', userID, { expires: new Date(Date.now() + 86400000*30) });
    }
    ...
    var options = {
        ...
        body: {
            ...
            data : {
                user: req.cookies[ 'demos_session' ] || '新用户（首次访问用户）',
                value: 'load'
            }
        }
    };
...
exports.redirectHtml = function (req, res) {
    var id = ObjectID().toString();
    res.redirect( '/html/' + id );
};

exports.redirectJs = function (req, res) {
    var id = ObjectID().toString();
    res.redirect( '/js/' + id );
};

exports.edit = function (req, res) {
    res.render( 'edit-html', { data : { _id : req.params.id } } );
};

exports.editJS = function (req, res) {
    res.render( 'edit-js', { data : { _id : req.params.id } } );
};

exports.result = function (req, res) {
    pageCode.getCodeById( req.params.id, function( err, result ){
        result ? res.send( result.codeText ) : res.send('404');
    } );
};
...
exports.getCodeInfo = function (req, res) {
    ...
    pageCode.getCodeById( id, function (err, result) {

        if (err) return tool.statError();
        ...
        if( result ){
            ...
            var obj = { codeText : '', userID: userID, type : type };
            if( type === 'html' ) obj.codeText = str;
            res.send( obj );
        }
    });
};
...
exports.postRegister = function (req, res) {
    ...
    if (mail && password && demos_session) {
        ...
        pageUser.register(mail, password, demos_session, function (err, result) {
            if( result ){
                tool.sendMail(mail, '激活您的Demos账户', html, function(error, info){
                    if(error){
...
exports.rmCode = function (req, res) {
    var id = req.params.id;
    pageCode.rmCodeById(id, function (err, info) {
        if( err ){
            res.status( 500 ).send(err);
        }else{
            res.send();
