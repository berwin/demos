...
exports.createCode = function( data, callback ){
    if( data.id && data.codeText ){
        ...
        codeDB.getCodeById( data.id, function( err, result ){
            if( !err && !result ){
                codeDB.insertCode( { _id : data.id, codeText : data.codeText, time : time }, function( err, data ){
                    callback( err, data, 1 );
                } );
            }
            if( result ){
                codeDB.updateCode( data.id, { codeText : data.codeText, time : time }, function( err, data ){
                    callback( err, data, 0 );
                } );
            }
        } );
